defmodule Sicktix.Email.TicketConfirm do
  import Swoosh.Email
  ...
  def build(%{event_id: event_id, owner: %{"user" => user_id}}) do
    with {:ok, %{name: event_name}} <-
           EventInterface.get_event(event_id),
      ...
      |> subject("Your tickets have been confirmed!")
      |> html_body(
        "<h1>Hello #{user_fn}</h1> <p>Your tickets for #{event_name} have been confirmed! We'll send you a reminder closer to the date of your event. Have fun!</p>"
      )
    else
      error ->
        Logger.error(
